...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    // Fetch-join product to avoid N+1 on list (exclude deleted products)
    @Query("select i from Inventory i join fetch i.product p where p.deleted = false")
    List<Inventory> findAllWithProduct();

    // Fetch-join product for low-stock query (exclude deleted products)
    @Query("select i from Inventory i join fetch i.product p where p.deleted = false and i.quantityAvailable < :threshold")
